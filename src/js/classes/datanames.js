...
// property "terms" for a list of terms to match against 
// method "GetTerm" to match a string to its closest term match 
 ...
 *  - x: values for this specific term
 ...
 * @see {@link delimiters} for delimiters that can be used in names (splitting makes sorting easier)
 */
const terms = [
    ...
    ['City',
        ...
        // common alt names 
        [
            'town', 'municipality', 'municipal', 'capital', 'township',
...
/**
 * Takes an input string, and uses {@link diceCoefficient Dice Coefficient} to
 * determine the {@link terms term} it most closely matches. Returns null if no match found.
 * @param {string} inputString String to compare against
 * @returns {string} Matching term if found. Null if no reasonable match found (or invalid)
 * @see {@link terms} for list of terms to search for
 * @see {@link coeffBounds} for values determining likelihood of a match 
 * @example GetTerm("location_latitude"); // returns "Latitude"
 */
export function GetTerm(inputString) {
    ...
    if (coeff.accuracy >= coeffBounds.accurate) {
...
function GetTermAndCoefficient(inputString) {
    ...
    if (mostAccurate.termIDs.length > 1) {
        ...
        if (mostAccurate.termIDs.length > 1) {
            ...
            let topTermAccuracy = coeffBounds.probable;
...
/** 
 * Creates a {@link diceCoefficient Dice Coefficient} object, 
 * with values for termName (default null) and accuracy (default -1) */
function DCObject(termName = null, accuracy = -1) { return { termName: termName, accuracy: accuracy }; }


/** Boundaries of dice-coefficient result matching */
const coeffBounds = {
    /** this value or higher is considered an accurate match */
    accurate: 0.92,
    /** this value or higher is considered a probable match */
    probable: 0.6,
}
